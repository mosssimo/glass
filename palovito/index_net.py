...
from sklearn_theano.base import (Convolution, MaxPool, Standardize, ZeroPad, Relu, fuse, Feedforward)
...
def _get_architecture(large_network=False, weights_and_biases=None,
    ...
    if large_network and not detailed:
        ...
        architecture = [
            ...
            Convolution(ws[7], bs[7],
                        activation='identity')]
        print "arch", ws[6].shape, type(ws[6]), bs[6].shape, type(bs[6])
...
def getSemanticLayer():
    params = pickle.load(file('net1_params.pkl'))
    hidden = params['hidden']
    W = hidden[0].transpose(1,0)
    b = hidden[1]
    sz = W.shape
    W.reshape(sz[0],sz[1],1,1)
    print 'me', W.shape, b.shape
    return  Convolution(W[7], b[7], activation='relu'),
...
class OverfeatIndexer(BaseEstimator, TransformerMixin):
    """
...
if __name__=="__main__":
    ...
    img = cv2.imread("/home/simon/data/palovito/images/B00009YQ6D.jpg")
    crop = simpleProcessImage(img)

    trans = OverfeatIndexer(force_reshape=False, output_layers=[-1])
    f = trans.transform(crop)
    print f[0].shape
